...
-- TeamSelf luôn đọc team hiện tại của player (dynamic)
local function TeamSelf() return tostring(Player.Team) end
...
-- Giữ team người dùng đã chọn, không ép về Pirates
getgenv().Team = getgenv().Team or tostring(Player.Team)
...
-- Hàm BringEnemy - gom quái cùng loại về vị trí player
local BringEnemyTweens = {}
BringEnemy = function()
    ...
    -- Lấy tên quái đang farm từ PosMon (quái mục tiêu hiện tại)
    local targetName = nil
    for _, v in pairs(workspace.Enemies:GetChildren()) do
        if v:FindFirstChild("HumanoidRootPart") and v.HumanoidRootPart.Position == PosMon then
            targetName = v.Name
            break
        end
    end
    -- Nếu không tìm được tên thì dùng tên của quái gần PosMon nhất
    if not targetName then
        local closestDist = math.huge
        for _, v in pairs(workspace.Enemies:GetChildren()) do
            if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                local d = (v.HumanoidRootPart.Position - PosMon).Magnitude
    ...
    for _, v in pairs(workspace.Enemies:GetChildren()) do
        -- Chỉ gom quái cùng tên, còn sống, trong radius 800
        if v.Name == targetName
            ...
            if dist <= 800 then
                ...
                -- Gom về ngay cạnh player (offset nhỏ để không chồng lên nhau)
                local offset = CFrame.new(playerPos) * CFrame.new(math.random(-3,3), 0, math.random(-3,3))
                local tweenTime = math.clamp(dist / 600, 0.05, 0.25)
                local info = TweenInfo.new(tweenTime, Enum.EasingStyle.Linear)
                if BringEnemyTweens[v] then
                    BringEnemyTweens[v]:Cancel()
                end
                local tw = TweenService:Create(v.HumanoidRootPart, info, {CFrame = offset})
                BringEnemyTweens[v] = tw
                tw:Play()
                tw.Completed:Connect(function()
                    BringEnemyTweens[v] = nil
                    -- Giữ quái đứng yên bằng BodyVelocity sau khi gom xong
                    if v and v.Parent and v:FindFirstChild("HumanoidRootPart") then
                        if not v.HumanoidRootPart:FindFirstChild("BV_Hold") then
...
_tp = function(target)
  ...
  local tweenInfo = TweenInfo.new(math.clamp(distance / 1000, 0.05, 0.2), Enum.EasingStyle.Linear)
...
-- Helper tele đến NPC rồi mua
local function BuyMeleeAtNpc(npcCFrame, invokeArgs)
    notween(npcCFrame)
    task.wait(0.5)
    replicated.Remotes.CommF_:InvokeServer(unpack(invokeArgs))
end
...
FightingShop:AddToggle("BlackLegMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Zeff (Restaurant/Baratie) - W1
            BuyMeleeAtNpc(CFrame.new(-1872.8, 128.1, 1389.7), {"BuyBlackLeg"})
        end
    end
})
...
FightingShop:AddToggle("FishManMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Fishman Karate Trainer - Underwater City W2
            BuyMeleeAtNpc(CFrame.new(-11609, 332, -8916), {"BuyFishmanKarate"})
        end
    end
})
...
FightingShop:AddToggle("ElectroMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Electro Trainer - Zou W2
            BuyMeleeAtNpc(CFrame.new(3724.5, 158.5, -3219.2), {"BuyElectro"})
        end
    end
})
...
FightingShop:AddToggle("DragonClawMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Blackbeard Reward
            BuyMeleeAtNpc(CFrame.new(-1246.9, 40.9, 4061.8), {"BlackbeardReward", "DragonClaw", "2"})
        end
    end
})
...
FightingShop:AddToggle("SuperHumanMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Martial Arts Master - W2 Colosseum
            BuyMeleeAtNpc(CFrame.new(411.9, 208.7, -3427.6), {"BuySuperhuman"})
        end
    end
})
...
FightingShop:AddToggle("DeathStepMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Dark Step Teacher - W1
            BuyMeleeAtNpc(CFrame.new(-1374.3, 36.5, 2266.7), {"BuyDeathStep"})
        end
    end
})
...
FightingShop:AddToggle("SharkmanMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Underwater - W3
            BuyMeleeAtNpc(CFrame.new(-9871.7, -894.1, -11954.2), {"BuySharkmanKarate"})
        end
    end
})
...
FightingShop:AddToggle("ElectricClawMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Previous Hero - W3
            BuyMeleeAtNpc(CFrame.new(-13556.8, 413.8, -7625.4), {"BuyElectricClaw"})
        end
    end
})
...
FightingShop:AddToggle("DragonTalonMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Uzoth - W3
            BuyMeleeAtNpc(CFrame.new(-9461.5, 19.4, 6252.1), {"BuyDragonTalon"})
        end
    end
})
...
FightingShop:AddToggle("GodHumanMeleeBuy", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Longma - W3
            BuyMeleeAtNpc(CFrame.new(-13010.5, 584.8, -7010.8), {"BuyGodhuman"})
        end
    end
})
...
FightingShop:AddToggle("SanguineArt", {
    ...
    Callback = function(Value)
        if Value then
            -- NPC: Sniper - Haunted Castle W3
            BuyMeleeAtNpc(CFrame.new(-9469.2, 168.2, 6023.4), {"BuySanguineArt"})
        end
    end
})
