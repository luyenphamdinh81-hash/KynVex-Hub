...
-- Hàm BringEnemy (smooth tween + gom nhiều quái hơn)
local BringEnemyTweens = {}
BringEnemy = function()
    if not _B then return end
    plr.SimulationRadius = math.huge
    local targetCFrame = CFrame.new(PosMon)
    for _, v in pairs(workspace.Enemies:GetChildren()) do
        if v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v.PrimaryPart then
            local dist = (v.PrimaryPart.Position - PosMon).Magnitude
            -- Tăng radius từ 300 → 800 để gom nhiều quái hơn
            if dist <= 800 then
                ...
                -- Dùng TweenService để kéo quái mượt mà thay vì teleport giật cục
                local tweenTime = math.clamp(dist / 400, 0.05, 0.3)
                ...
                local tw = TweenService:Create(v.PrimaryPart, info, {CFrame = targetCFrame})
